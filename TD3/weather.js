...
class WeatherInfo {
    ...
    getDateTime() {
      ...
      return (new Date(dateParts[2], dateParts[1] - 1, dateParts[0])).toLocaleDateString('fr-FR', options);
    }
...
class Weather {
    ...
    refresh() {
        let search = document.getElementsByClassName("searchbox")[0].value;
        fetch("https://www.prevision-meteo.ch/services/json/"+search).then( response => {
...
// testing
    document.getElementsByClassName("searchbox")[0].value = "perigueux";
    weather.refresh();
